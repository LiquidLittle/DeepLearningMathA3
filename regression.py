...
def fit_regression_model(X, y):
    ...
    input_features = X.size(dim=1) # extract the number of features from the input `shape` of X
    print(input_features)
    output_features = y.size(dim=1)# extract the number of features from the output `shape` of y
    ...
    for epoch in range(1, num_epochs):
        ...
        if previos_loss - loss < 0.001: # Change this condition to stop the training when the loss is not changing much.
            break
